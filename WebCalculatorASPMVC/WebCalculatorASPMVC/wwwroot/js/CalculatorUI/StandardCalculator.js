...
{
    /** (Updated: 1-11-2020)
     * This section is the MouseDown/Keyboard/Touch Events and Math Logic for: 
     * Standard (Sequential-Input) Calculations.
     * 
     ...
     * Note: Operation()s is to configure a string for Calculation() to interpret into a finalized equation for eval();
     */
    ...
    //...
    key(value) {
        this.number(value);//KeysPressed
        this.decimal(value);//KeysPressed
        this.operation(value);//KeysPressed
        this.equalsOperation(value);//KeysRecorder
    }
    //...
    ...
    calculation() {
        ...
        //...
        var lastChar = this.KeysRecorder[this.KeysRecorder.length - 1];//Get the Last Character in the KeysRecorder.
        //...

        //...
        //First Condition: the last character in the finalized string is an Operator. Remove it & continue the calculation.
        if (this.operatorMatch(lastChar) !== false && Object.keys(this.KeysPressed).length === 0) {//This conditions if the user has "12+12+" then hits equal sign.
        ...
        } else if (this.operatorMatch(lastChar) !== false && Object.keys(this.KeysPressed).length > 0) {//This conditions if the user has "12+12" then hits equal sign.
    ...
    clearDisplay() {
        ...
        document.getElementById("standard-calculator-textarea-display").setAttribute("placeholder", "");//Empty the <textArea> to default.
    ...
    decimal(value) {
        ...
        */
        if (value === ".") {
            var previousKeys = this.KeysPressed[this.KeysPressed.length - 1];//returns single character string from the KeysPressed[].
    ...
    display() {
        ...
        //document.getElementById("standard-calculator-textarea-display").scrollTop = document.getElementById("standard-calculator-textarea-display").scrollHeight;//Forces the scroll to the bottom of the latest User Input in the <TextArea>.
    }
    //...
    ...
    number(value) {
        /**
        * The KeysPressed[] is used for Numerical Inputs to create calculator keyspressed value(s) into Number(s).
        */
        if (!isNaN(value)) {
    ...
    operation(value) {
        /** 
        * Everytime a User selects a Math Operator, the keysRecorder.push( this.keysPressed[as a string] ); & reset the KeysPressed[emptyObject];.
        * KeysPressed Should only be digits and a decimal "1.23". Adding numbers&decimal and the Math Operator "123+".
        * Standard Calculators will calculate in Sequence after the 2nd Math Operator i.e."10+10+" will become "20+"(display) for the user and ready for KeysPressed[] and decimal() to event again.
        * It Calculates immediately without considering "PEMDAS/BEDMAS" in Algerbraic Expressions. So this is basically a Loop for Standard Calculations everytime a Math Operator is user pressed.
        * I.E. Inputting "Left to Right" in 123 + 123 * 2. It will do 123 + 123 first, then multiply by 2. eval() has no rules of Pemdas nor does StandCalculations.
        */
        ...
        if (this.operatorMatch(lastChar) !== false && Object.keys(this.KeysPressed).length === 0) {
        ...
        } else if (this.operatorMatch(value) !== false && Object.keys(this.KeysPressed).length !== 0) {
            ...
            if (Object.keys(this.KeysRecorder).length >= 3) {
                var calculateString = this.KeysRecorder[0] + this.KeysRecorder[1] + this.KeysRecorder[2];//Get Elements in the KeysRecorder[] i.e. "[123], [+], [123]".
    ...
    keyBoardEventCommand(e) {
        ...
        */
        switch (e.keyCode) {
            ...
            case 46://Delete
                this.clearDisplay();
                ScientificCalculator.prototype.clearDisplay();
                break;
            case 8://BackSpace
                this.clearDisplay();
...
var standard = new StandardCalculator();
